...
export default function parseCompeDetail(body: string): CompeDetail | null {
    ...
    const compeDetail: CompeDetail = {
        ...
        hostNickname: $("#compeDetail > ul.festivalThumbList > li > section > aside > div > ul > li:nth-child(2)").text().replace('主催：', '').trim(),
        ...
        startDate: new CompeDate($('#compeDetail > ul.festivalThumbList > li > section > aside > div > ul > li:nth-child(4)').text().replace('期間：', '').replace('～', '').trim()),
        endDate: new CompeDate($('#compeDetail > ul.festivalThumbList > li > section > aside > div > ul > li:nth-child(5)').text().trim()),
        songList: []
    }
