...
export default function parseDaniData(responseDatas) {
    if (Array.isArray(responseDatas)) {
        return responseDatas.map(parseDaniResponseData).filter(e => e !== null);
    }
    else {
        return parseDaniResponseData(responseDatas);
    }
}
function parseDaniResponseData(responseData) {
